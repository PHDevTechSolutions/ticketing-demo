...
function getPriorityStyle(priority?: string): string {
    switch (priority) {
        ...
        default:         return "text-[#e5e5d0]/50 border-[#2a2a1a] bg-transparent";
    }
}
...
export const Received: React.FC<RequestProps> = ({ referenceid, fullname, dateCreatedFilterRange }) => {
    ...
    if (errorActivities) {
        return (
            <div className="border border-[#ef4444]/30 bg-[#ef4444]/5 p-4 space-y-3">
                ...
                <p className="text-[10px] font-mono text-[#e5e5d0]/50">Please check your internet connection or try again later.</p>
    ...
    return (
        <div className="w-full flex flex-col" style={{ border: "1px solid rgba(255,255,255,0.07)", backgroundColor: "#080c10" }}>

            {/* Toolbar */}
            <div className="flex items-center justify-between gap-3 px-4 py-3" style={{ borderBottom: "1px solid rgba(255,255,255,0.07)", backgroundColor: "rgba(255,255,255,0.02)" }}>
                <input
                    type="text"
                    placeholder="> search tickets..."
                    className="text-[11px] font-mono text-[#e5e5d0]/80 placeholder:text-white/20 px-3 py-1.5 w-full max-w-[360px] focus:outline-none"
                    style={{ backgroundColor: "#080c10", border: "1px solid rgba(255,255,255,0.07)" }}
                    value={search}
                    onChange={(e) => { setSearch(e.target.value); setPage(1); setSelectedIds(new Set()); }}
                />
                <div className="flex items-center gap-2 shrink-0">
                    <button onClick={() => setFilterOpen(true)} className="text-[10px] font-mono uppercase tracking-widest px-3 py-1.5 text-white/50 hover:text-[#f97316] transition-colors" style={{ border: "1px solid rgba(255,255,255,0.07)", backgroundColor: "#080c10" }}>
                        [ Filters ]
                    </button>
                    <button onClick={downloadCSV} className="text-[10px] font-mono uppercase tracking-widest px-3 py-1.5 text-white/50 hover:text-[#f97316] transition-colors" style={{ border: "1px solid rgba(255,255,255,0.07)", backgroundColor: "#080c10" }}>
                        [ CSV ]
                    </button>
                    {selectedIds.size > 0 && (
                        <button onClick={handleDeleteSelected} className="text-[10px] font-mono uppercase tracking-widest px-3 py-1.5 text-[#ef4444] hover:bg-[#ef4444]/10 transition-colors" style={{ border: "1px solid rgba(239,68,68,0.3)", backgroundColor: "#080c10" }}>
                            [ Delete {selectedIds.size} ]
                        </button>
                    )}
                    <button onClick={() => { resetForm(); setOpen(true); }} className="text-[10px] font-mono uppercase tracking-widest px-3 py-1.5 text-[#f97316] hover:bg-[#f97316]/10 transition-colors" style={{ border: "1px solid rgba(249,115,22,0.4)", backgroundColor: "#080c10" }}>
                        [ + New Ticket ]
                    </button>
                </div>
            </div>
            ...
            {loadingActivities ? (
                <div className="flex items-center gap-2 justify-center py-12">
                    ...
                    <span className="text-[10px] font-mono text-[#6b6b4a]/50 ml-1 uppercase tracking-widest">Loading...</span>
                </div>
            ) : filteredActivities.length === 0 ? (
                <div className="text-[10px] font-mono text-[#6b6b4a]/50 uppercase tracking-widest text-center py-12">
                    ...
                    <table className="w-full text-[10px] font-mono whitespace-nowrap">
                        ...
                        <tbody>
                            {paginatedActivities.map((item) => (
                                <tr key={item.id} className="transition-colors" style={{ borderBottom: "1px solid rgba(255,255,255,0.05)" }}
                                    ...
                                    <td className="px-3 py-2 text-[#e5e5d0]/50"><span className="flex items-center gap-1"><Clock3 className="w-2.5 h-2.5" />{computeDuration(item.date_created, item.date_closed, item.status)}</span></td>
                                    ...
                                    <td className="px-3 py-2 text-[#e5e5d0]/60 uppercase">{item.requestor_name || "-"}</td>
                                    <td className="px-3 py-2 text-[#e5e5d0]/50">{item.department || "-"}</td>
                                    <td className="px-3 py-2 text-[#e5e5d0]/50">{item.request_type || "-"}</td>
                                    <td className="px-3 py-2 text-[#e5e5d0]/50">{item.type_concern || "-"}</td>
                                    <td className="px-3 py-2 text-[#e5e5d0]/50">{item.mode || "-"}</td>
                                    <td className="px-3 py-2 text-[#e5e5d0]/50">{item.group_services || "-"}</td>
                                    <td className="px-3 py-2 text-[#e5e5d0]/60 uppercase">{item.technician_name || "-"}</td>
                                    <td className="px-3 py-2 text-[#e5e5d0]/50">{item.site || "-"}</td>
                                    <td className="px-3 py-2 text-[#e5e5d0]/50">{item.date_scheduled || "-"}</td>
                                    <td className="px-3 py-2 text-[#e5e5d0]/60 uppercase">{item.processed_by || "-"}</td>
                                    <td className="px-3 py-2 text-[#e5e5d0]/60 uppercase">{item.closed_by || "-"}</td>
                                    <td className="px-3 py-2 text-[#e5e5d0]/50 max-w-[160px] truncate">{item.remarks || "-"}</td>
                                    <td className="px-3 py-2 text-[#e5e5d0]/40">{formatDateCreated(item.date_created)}</td>
                                    <td className="px-3 py-2 text-[#e5e5d0]/40">{formatDateCreated(item.date_closed)}</td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
            ...
            {/* Pagination */}
            {filteredActivities.length > 0 && (
                <div className="flex justify-end px-4 py-3" style={{ borderTop: "1px solid rgba(255,255,255,0.07)", backgroundColor: "rgba(255,255,255,0.02)" }}>
                    <div className="flex items-center gap-3">
                        <button
                            onClick={() => { if (page > 1) setPage(page - 1); }}
                            disabled={page <= 1}
                            className="text-[9px] font-mono uppercase tracking-widest px-3 py-1.5 text-white/40 hover:text-[#f97316] transition-colors disabled:opacity-30 disabled:cursor-not-allowed"
            ...
            <Dialog open={filterOpen} onOpenChange={setFilterOpen}>
                <DialogContent className="max-w-sm rounded-none p-0" style={{ backgroundColor: "#080c10", border: "1px solid rgba(255,255,255,0.1)" }}>
                    <DialogHeader className="px-5 py-4" style={{ borderBottom: "1px solid rgba(255,255,255,0.07)" }}>
                        ...
                        <DialogDescription className="text-[10px] font-mono text-white/30">Narrow down the ticket list.</DialogDescription>
                    </DialogHeader>
                    <div className="px-5 py-4 space-y-4">
                        {[
                            ...
                            <div key={id}>
                                ...
                                <select id={id} className="w-full text-[#e5e5d0]/70 text-[10px] font-mono px-3 py-1.5 focus:outline-none" style={{ backgroundColor: "#080c10", border: "1px solid rgba(255,255,255,0.07)" }} value={value} onChange={(e) => setter(e.target.value)}>
                    ...
                    <div className="flex justify-end gap-2 px-5 py-4" style={{ borderTop: "1px solid rgba(255,255,255,0.07)" }}>
                        <button onClick={() => { setStatusFilter(""); setRequestTypeFilter(""); setPriorityFilter(""); }} className="text-[9px] font-mono uppercase tracking-widest px-3 py-1.5 text-white/40 hover:text-[#f97316] transition-colors" style={{ border: "1px solid rgba(255,255,255,0.07)" }}>
                            Clear
                        </button>
                        <button onClick={() => setFilterOpen(false)} className="text-[9px] font-mono uppercase tracking-widest px-3 py-1.5 text-[#f97316] hover:bg-[#f97316]/10 transition-colors" style={{ border: "1px solid rgba(249,115,22,0.4)" }}>
                            Apply
                        </button>
                    </div>
                </DialogContent>
            ...
            <Dialog open={confirmDeleteOpen} onOpenChange={setConfirmDeleteOpen}>
                <DialogContent className="max-w-sm rounded-none p-0" style={{ backgroundColor: "#080c10", border: "1px solid rgba(239,68,68,0.3)" }}>
                    <DialogHeader className="px-5 py-4" style={{ borderBottom: "1px solid rgba(239,68,68,0.15)" }}>
                        <DialogTitle className="text-[11px] font-mono font-bold text-[#ef4444] uppercase tracking-[0.2em]">✕ Confirm Deletion</DialogTitle>
                        <DialogDescription className="text-[10px] font-mono text-white/40">
                            Delete <strong className="text-white/70">{selectedIds.size}</strong> selected item{selectedIds.size > 1 ? "s" : ""}? This cannot be undone.
                        </DialogDescription>
                    </DialogHeader>
                    <div className="flex justify-end gap-2 px-5 py-4">
                        <button onClick={() => setConfirmDeleteOpen(false)} className="text-[9px] font-mono uppercase tracking-widest px-3 py-1.5 text-white/40 hover:text-[#f97316] transition-colors" style={{ border: "1px solid rgba(255,255,255,0.07)" }}>
                            Cancel
                        </button>
                        <button onClick={confirmDeletion} className="text-[9px] font-mono uppercase tracking-widest px-3 py-1.5 text-[#ef4444] hover:bg-[#ef4444]/10 transition-colors" style={{ border: "1px solid rgba(239,68,68,0.4)" }}>
                            Delete
                        </button>
                    </div>
                </DialogContent>
